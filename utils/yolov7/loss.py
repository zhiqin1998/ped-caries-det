# Loss functions from https://github.com/WongKinYiu/yolov7/blob/main/utils/loss.py
# modified ComputeLossOTA to optionally allow weights for EARL
...
class ComputeLossOTA:
    # Compute losses
    def __init__(self, model, autobalance=False, weighted=False):
        ...
        BCEcls = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([h['cls_pw']], device=device), reduction='none' if weighted else 'mean')
        BCEobj = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([h['obj_pw']], device=device), reduction='none' if weighted else 'mean')
    ...
    def __call__(self, p, targets, imgs, weights=None):  # predictions, targets, model
        device = targets.device
        lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
        bs, as_, gjs, gis, targets, anchors, weights = self.build_targets(p, targets, imgs, weights)
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            tobj = torch.zeros_like(pi[..., 0], device=device)  # target obj
            lobj_weight = torch.ones_like(tobj)
            ...
            if n:
                ...
                lbox += ((1.0 - iou) * selected_weights.unsqueeze(1)).mean()  # iou loss

                # Objectness
                tobj[b, a, gj, gi] = (1.0 - self.gr) + self.gr * iou.detach().clamp(0).type(tobj.dtype)  # iou ratio
                lobj_weight[b, a, gj, gi] = selected_weights
                ...
                if self.nc > 1:  # cls loss (only if multiple classes)
                    ...
                    if self.weighted:
                        t_lcls = torch.mean(self.BCEcls(ps[:, 5:], t) * selected_weights.unsqueeze(1))
                    else:
                        t_lcls = self.BCEcls(ps[:, 5:], t)
                    lcls += t_lcls  # BCE
                ...
                #     [file.write('%11.5g ' * 4 % tuple(x) + '\n') for x in torch.cat((txy[i], twh[i]), 1)]
            if self.weighted:
                obji = torch.mean(self.BCEobj(pi[..., 4], tobj) * lobj_weight)
            else:
                obji = self.BCEobj(pi[..., 4], tobj)
            lobj += obji * self.balance[i]  # obj loss
            if self.autobalance:
    ...
    def build_targets(self, p, targets, imgs, weights=None):
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            this_weights = this_weights[matched_gt_inds]
            ...
            for i in range(nl):
                ...
                matching_weights[i].append(this_weights[layer_idx])
        ...
        for i in range(nl):
            if matching_targets[i] != []:
                ...
                matching_weights[i] = torch.cat(matching_weights[i], dim=0)
            else:
                ...
                matching_weights[i] = torch.tensor([], device='cuda:0', dtype=torch.int64)

        return matching_bs, matching_as, matching_gjs, matching_gis, matching_targets, matching_anchs, matching_weights
